...
def valid(p):
 try:
  m=json.loads((p/'metrics.json').read_text());d=json.loads((p/'metadata.json').read_text());z=np.load(p/'test_predictions.npz',allow_pickle=False);return all((p/x).is_file() for x in ('best.pt','config.json','history.csv')) and d['status']=='complete' and d['checkpoint_reloaded'] is True and d['test_used_for_selection'] is False and d['test_model_inference_count']==1 and d['test_time_calibration'] is False and d['model_inputs']=='pixels_only' and m['test_model_inference_count']==1 and len(z['image_indices'])==len(z['image_ids'])==1425 and all(math.isfinite(float(m[k])) for k in ('test_mae_months','test_mse_months2','test_rmse_months','test_r2','test_plcc','test_srocc'))
